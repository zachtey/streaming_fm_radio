...
module demod_tb;
    ...
    integer r, idx;
    integer out_count, err_count, compare_count;
    reg [31:0]        word_tmp;
    ...
    task automatic load_i_file;
        begin
            n_i = 0;
            fd_i = $fopen("gold_01_channel_i_fir.txt", "r");
            if (fd_i == 0) begin
                $display("ERROR: could not open gold_01_channel_i_fir.txt");
                $finish;
            end
    ...
    task automatic load_q_file;
        begin
            n_q = 0;
            fd_q = $fopen("gold_01_channel_q_fir.txt", "r");
            if (fd_q == 0) begin
                $display("ERROR: could not open gold_01_channel_q_fir.txt");
                $finish;
            end
    ...
    task automatic load_golden_file;
        begin
            n_golden = 0;
            fd_golden = $fopen("gold_02_demod.txt", "r");
            if (fd_golden == 0) begin
                $display("ERROR: could not open gold_02_demod.txt");
                $finish;
            end
    ...
    initial begin
        ...
        if (n_golden != n_i) begin
            $display("WARNING: golden count (%0d) != input count (%0d)", n_golden, n_i);
        end
        ...
        // Feed one sample, wait for one output
        ...
        $display("Compared outputs     : %0d", compare_count);
        ...
        if (compare_count > 0) begin
            ...
            $display("Avg signed diff      : %0f", $itor(sum_signed_diff) / compare_count);
            $display("Avg abs diff         : %0f", $itor(sum_abs_diff) / compare_count);
    ...
    always @(posedge clk) begin
        if (!rst && demod_valid_out) begin
            ...
            if (out_count >= n_golden) begin
                ...
                if (abs_diff_int <= 1)   within_1_count   = within_1_count + 1;
                if (abs_diff_int <= 2)   within_2_count   = within_2_count + 1;
                if (abs_diff_int <= 4)   within_4_count   = within_4_count + 1;
                if (abs_diff_int <= 8)   within_8_count   = within_8_count + 1;
                if (abs_diff_int <= 16)  within_16_count  = within_16_count + 1;
                if (abs_diff_int <= 32)  within_32_count  = within_32_count + 1;
                if (abs_diff_int <= 64)  within_64_count  = within_64_count + 1;
                if (abs_diff_int <= 128) within_128_count = within_128_count + 1;
